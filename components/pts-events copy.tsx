...
const AirCanadaPTSEventDrawer = () => {
  ...
  const timelineEnd = new Date(flightData.categories[flightData.categories.length-1].events[flightData.categories[flightData.categories.length-1].events.length-1].scheduledStart);
  ...
  return (
    <div className="h-full w-full max-w-md bg-gray-50 shadow-lg flex flex-col overflow-auto">
      {/* Current time vertical line overlay removed from here */}
      {/* Header */}
                ...
                {category.events.map((event) => (
                  <div key={event.id} className="mb-5">
                    ...
                    <div className="relative h-12 bg-gray-100 rounded-lg overflow-hidden my-3 border border-gray-200 shadow-sm">
                      ...
                      <div 
                        className="absolute top-0 bottom-0 border-l-2 border-red-600 border-dashed z-30"
                        style={{ left: `${calculatePosition(flightData.currentTime)}%` }}
                      ></div>
                        ...
                        <div 
                          ...
                          style={{
                            ...
                            width: event.actualEnd 
                              ...
                              : event.progress 
                                ? `${(calculatePosition(event.scheduledEnd) - calculatePosition(event.actualStart)) * (event.progress / 100)}%`
                                : '10px'
                          }}
                        >
