...
export default function HubMap({ onSelectAircraft, satelliteView = false, gateCoordinates = null }: HubMapProps = {}) {
        ...
        Papa.parse(csvText, {
          ...
          complete: (results) => {
            const data = results.data.filter(row => 
              row.latitude && row.longitude && row.seconds !== undefined
            );
  ...
  const initializeVehicleMarker = useCallback(() => {
    ...
    el.innerHTML = `
      <div class="gse-vehicle-icon" style="transform: rotate(${initialHeading}deg)">
        <img src="${resolvedTheme === "light" ? "/gse-vehicle-icon-dark.svg" : "/gse-vehicle-icon.svg"}" 
            width="24" height="16" alt="GSE Vehicle" />
      </div>
    `;
  ...
  useEffect(() => {
    ...
    // If following the vehicle, update the map view
    if (isVehicleAnimating && map.current) {
      map.current.panTo([currentPoint.longitude, currentPoint.latitude], { duration: 1000 });
    }
  ...
  const addMarkersToMap = useCallback(() => {
    ...
    // Add GSE vehicle markers
    // GSE_VEHICLES.forEach((vehicle) => {
    //   // Create a DOM element for the marker
    //   const el = document.createElement("div")
    //   el.className = "gse-vehicle-marker"
    ...
    //   img.onerror = () => {
    //     // If image fails to load, use inline SVG as fallback
    //     el.innerHTML = `
    //         <div class="gse-vehicle-icon" style="transform: rotate(${vehicle.heading}deg)">
    //           <svg xmlns="http://www.w3.org/2000/svg" width="24" height="16" viewBox="0 0 40 24">
    //             <rect x="0" y="0" width="28" height="20" rx="4" fill="${resolvedTheme === "light" ? "#1e293b" : "#B8C4CF"}"/>
    //             <rect x="28" y="4" width="12" height="12" rx="2" fill="${resolvedTheme === "light" ? "#1e293b" : "#B8C4CF"}"/>
    //           </svg>
    //         </div>
    //       `

    //     // Add marker to map after setting fallback
    //     addMarkerToMap()
    //   }

    //   // Set up load handler
    //   img.onload = () => {
    //     // Once image is loaded, set the HTML with the image
    //     el.innerHTML = `
    //         <div class="gse-vehicle-icon">
    //           <img src="${resolvedTheme === "light" ? "/gse-vehicle-icon-dark.svg" : "/gse-vehicle-icon.svg"}" width="24" height="16" alt="GSE Vehicle" />
    //         </div>
    //       `

    //     // Add marker to map after image is loaded
    //     addMarkerToMap()
    //   }
    ...
    //     // Add click event handler
    //     el.addEventListener("click", () => {
    //       console.log(`GSE vehicle ${vehicle.id} clicked`)
    ...
    //       // console.log(`Selected GSE Vehicle ${vehicle.id}`)
    //       console.log(`Vehicle 1111 : ${selectedGseVehicle} Drawer Open ${gseVehicleDrawerOpen}`)

    //       // Fly to the vehicle position
    //       flyToLocation(vehicle.position, {
    //         zoom: 18.5,
    //         pitch: 30,
    //         duration: 1500,
    //       })
